...
import com.ewisselectronic.sulama.sulamacore.utils.Strings;
...
public final class Log {
    ...
    public static Path configureLogFile(Path _logFile, boolean _ignoreFileLogging) throws IOException {
        ...
        if (_logFile == null) {
            // setting default log in "current-working-directory/log" directory.
            Path logRootDir = Paths.get(".").normalize().resolve("log");
            Files.createDirectories(logRootDir);
            // generate a unique name from date time information.
